...
public class DistributedCacheRateLimitStorage : IRateLimitStorage
{
    ...
    public void Set(string id, RateLimitCounter counter, TimeSpan expirationTime)
        => _memoryCache.SetString(id, JsonSerializer.Serialize(counter, OcelotSerializerOptions.Web), new DistributedCacheEntryOptions().SetAbsoluteExpiration(expirationTime));
