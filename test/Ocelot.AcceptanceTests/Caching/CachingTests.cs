    ...
    public sealed class CachingTests : IDisposable
    {
        ...
        public void Should_return_cached_response()
        {
            ...
            this.Given(x => x.GivenThereIsAServiceRunningOn($"http://localhost:{port}", HttpStatusCode.OK, HelloLauraContent, null, null))
                ...
                .And(x => _steps.ThenTheResponseBodyShouldBeAsync(HelloLauraContent))
        ...
        public void Should_not_return_cached_response_as_ttl_expires()
        {
            ...
            this.Given(x => x.GivenThereIsAServiceRunningOn($"http://localhost:{port}", HttpStatusCode.OK, HelloLauraContent, null, null))
                ...
                .When(x => _steps.WhenIGetUrlOnTheApiGatewayAsync("/"))
                .Then(x => _steps.ThenTheStatusCodeShouldBe(HttpStatusCode.OK))
                .And(x => _steps.ThenTheResponseBodyShouldBeAsync(HelloTomContent))
                .BDDfy();
        }
        ...
        public void Should_return_different_cached_response_when_request_body_changes_and_EnableContentHashing_is_true(bool asGlobalConfig)
        {
            ...
            this.Given(x => x.GivenThereIsAnEchoServiceRunningOn($"http://localhost:{port}"))
                .And(x => _steps.GivenThereIsAConfiguration(configuration))
                .And(x => _steps.GivenOcelotIsRunning())
                .When(x => _steps.WhenIPostUrlOnTheApiGatewayAsync("/", new StringContent(testBody1String, Encoding.UTF8, "application/json")))
                .Then(x => _steps.ThenTheStatusCodeShouldBe(HttpStatusCode.OK))
                .And(x => _steps.ThenTheResponseBodyShouldBeAsync(testBody1String))
                .When(x => _steps.WhenIPostUrlOnTheApiGatewayAsync("/", new StringContent(testBody2String, Encoding.UTF8, "application/json")))
                .Then(x => _steps.ThenTheStatusCodeShouldBe(HttpStatusCode.OK))
                .And(x => _steps.ThenTheResponseBodyShouldBeAsync(testBody2String))
                .When(x => _steps.WhenIPostUrlOnTheApiGatewayAsync("/", new StringContent(testBody1String, Encoding.UTF8, "application/json")))
                .Then(x => _steps.ThenTheStatusCodeShouldBe(HttpStatusCode.OK))
                .And(x => _steps.ThenTheResponseBodyShouldBeAsync(testBody1String))
                .When(x => _steps.WhenIPostUrlOnTheApiGatewayAsync("/", new StringContent(testBody2String, Encoding.UTF8, "application/json")))
