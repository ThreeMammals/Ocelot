...
- :doc:`../features/servicediscovery`: Introducing a new feature for "*Customization of services creation*" in two primary service discovery providers:
  ``Consul`` (:ref:`Consul Service Builder <sd-consul-service-builder>`) and ``Kube`` (:ref:`Downstream Scheme with Port Names <k8s-downstream-scheme-vs-port-names>`), developed by `@raman-m`_.

  The customization for both ``Consul`` and ``Kube`` providers in service creation is achieved through the overriding of virtual methods in default implementations (the ``DefaultConsulServiceBuilder`` and ``KubeServiceBuilder`` classes).
  The recommendation was to separate the provider's logic and introduce ``public virtual`` and ``protected virtual`` methods in concrete classes, enabling:
...
- :doc:`../features/routing`: Introducing the new ":ref:`Upstream Headers <routing-upstream-headers>`" feature by `@jlukawska`_. ⭐
...
- :doc:`../features/configuration`: Introducing the ":ref:`Downstream Http Version Policy <config-version-policy>`" feature by `@ibnuda`_.
...
- :doc:`../features/configuration`: New features are ":ref:`DownstreamHttpVersionPolicy <config-version-policy>`" by `@ibnuda`_ and ":ref:`config-route-metadata`" by `@vantm`_.
- :doc:`../features/servicediscovery`: New feature is "*Customization of services creation*" aka ":ref:`Consul Service Builder <sd-consul-service-builder>`" and ":ref:`Downstream Scheme with Port Names <k8s-downstream-scheme-vs-port-names>`" by `@raman-m`_.
- :doc:`../features/routing`: New feature is ":ref:`Upstream Headers <routing-upstream-headers>`" by `@jlukawska`_.
- :doc:`../features/qualityofservice`: The team has decided to remove the Polly V7 policies logic and the corresponding Ocelot ``AddPollyV7`` extensions (referenced in PR `2079`_).
  ...
  See more detaild below in ":ref:`extra-packages`" paragraph.
...
See `all bugs <https://github.com/ThreeMammals/Ocelot/issues?q=is%3Aissue%20state%3Aclosed%20label%3Abug%20label%3A%22Spring%2724%22>`_ of the `Spring'24 <https://github.com/ThreeMammals/Ocelot/milestone/6>`_ milestone.

Documentation Summary
^^^^^^^^^^^^^^^^^^^^^

- :doc:`../features/caching`: New ":ref:`cch-enablecontenthashing-option`" and ":ref:`cch-global-configuration`" sections
- :doc:`../features/configuration`: New ":ref:`DownstreamHttpVersionPolicy <config-version-policy>`" and ":ref:`config-route-metadata`" sections
- :doc:`../features/kubernetes`: New ":ref:`Downstream Scheme with Port Names <k8s-downstream-scheme-vs-port-names>`" section
...
- :doc:`../features/routing`: New ":ref:`Upstream Headers <routing-upstream-headers>`" section
- :doc:`../features/servicediscovery`: New ":ref:`Consul Service Builder <sd-consul-service-builder>`" and ":ref:`Downstream Scheme with Port Names <k8s-downstream-scheme-vs-port-names>`" sections
