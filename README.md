...
At the moment the only supported service discovery provider is Consul. There is an IServiceDiscoveryProvider
interface that can be implemented for any other service discovery providers.   

The following is required in the GlobalConfiguration. The Provider is required and if you do not 
specify a host and port the Consul default will be used.
